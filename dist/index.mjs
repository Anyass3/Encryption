...
function _encrypt({ publicKeyHex, publicKey }, msgParams, version) {
    switch (version) {
        case 'x25519-xsalsa20-poly1305': {
            ...
            // encrypt
            const encryptedMessage = N.box(msgParamsUInt8Array, nonce, pubKeyUInt8Array, ephemeralKeyPair.secretKey);
...
function _decrypt(encryptedData, receiverPrivateKey) {
    switch (encryptedData.version) {
        case 'x25519-xsalsa20-poly1305': {
            // string to buffer to UInt8Array
            const recieverPrivateKeyUint8Array = hexToBuffer(receiverPrivateKey);
            const recieverEncryptionPrivateKey = N.box.keyPair.fromSecretKey(recieverPrivateKeyUint8Array).secretKey;
            ...
            // decrypt
            const decryptedMessage = N.box.open(ciphertext, nonce, ephemPublicKey, recieverEncryptionPrivateKey);
...
const sign = (data, signSecretKey, encoding = 'hex') => {
...
const verifySignature = (signedData, signPublicKey, encoding = 'hex') => {
    try {
        const data = N.sign.open(encoding == 'base64' ? N.util.decodeUTF8(signedData) : hexToBuffer(signedData), hexToBuffer(signPublicKey));
...
const encrypt = async (message = 'message', publicKeyHex) => {
    let publicKey;
    if (!publicKeyHex)
        publicKey = await getMMPublicKey();
    if (!publicKey && !publicKeyHex) {
        console.error('NO Public Key');
        return;
    }
    const encrypted = _encrypt({ publicKey, publicKeyHex }, { data: message }, 'x25519-xsalsa20-poly1305');
...
export { N, _decrypt, _encrypt, bufferToHex, decrypt, encrypt, getMMPublicKey, hexToBuffer, isMetaMask, keyPair, random, sign, signMultiple, verifySignature, verifySignatures };
